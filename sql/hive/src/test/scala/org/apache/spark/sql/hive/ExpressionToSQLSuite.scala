...
class ExpressionToSQLSuite extends SQLBuilderTest with SQLTestUtils {
  ...
  test("string functions") {
    ...
    checkSqlGeneration("SELECT ltrim(' ', '  SparkSql ')")
    checkSqlGeneration("SELECT ltrim('S', 'SparkSqlSSS')")
    // wait for resolution of JIRA SPARK-12719 SQL Generation for Generators
    ...
    checkSqlGeneration("SELECT rtrim(' ', '  SparkSql ')")
    checkSqlGeneration("SELECT rtrim('S', 'SparkSqlSSS')")
    ...
    checkSqlGeneration("SELECT trim(BOTH 'S' FROM 'SparkSqlS')")
    checkSqlGeneration("SELECT trim(LEADING ' ' FROM '  SparkSql ')")
    checkSqlGeneration("SELECT trim(LEADING 'S' FROM 'SSSparkSqlS')")
    checkSqlGeneration("SELECT trim(TRAILING ' ' FROM '  SparkSql ')")
    checkSqlGeneration("SELECT trim(TRAILING 'S' FROM 'SSSparkSqlSSS')")
