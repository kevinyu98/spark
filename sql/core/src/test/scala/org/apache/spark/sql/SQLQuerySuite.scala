...
class SQLQuerySuite extends QueryTest with SharedSQLContext {
  ...
  test("TRIM function") {
    ...
    checkAnswer(
      sql("SELECT trim(LEADING 'on' FROM k) FROM trimTb"),
      Row("eo") :: Nil)
    ...
    Seq(("oneo", 1)).toDF("k", "v").registerTempTable("trimTb")
    checkAnswer(
      sql("SELECT trim(TRAILING 'eo' FROM k) FROM trimTb"),
      Row("on") :: Nil)
    ...
    Seq(("oneo", 1)).toDF("k", "v").registerTempTable("trimTb")
    checkAnswer(
      sql("SELECT trim(BOTH 'on' FROM k) FROM trimTb"),
      Row("eo") :: Nil)

    Seq(("oneo", 1)).toDF("k", "v").registerTempTable("trimTb")
    checkAnswer(
      sql("SELECT trim(BOTH 'eo' FROM k) FROM trimTb"),
      Row("on") :: Nil)
